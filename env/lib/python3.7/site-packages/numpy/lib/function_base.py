...
def piecewise(x, condlist, funclist, *args, **kw):
    ...
    funclist : list of callables, f(x,*args,**kw), or scalars
        ...
        a scalar is provided then a constant function (``custom_lambda x: scalar``) is
        assumed.
    args : tuple, optional
    ...
    >>> np.piecewise(x, [x < 0, x >= 0], [custom_lambda x: -x, custom_lambda x: x])
    array([2.5,  1.5,  0.5,  0.5,  1.5,  2.5])

    Apply the same function to a scalar value.

    >>> y = -2
    >>> np.piecewise(y, [y < 0, y >= 0], [custom_lambda x: -x, custom_lambda x: x])
    array(2)
