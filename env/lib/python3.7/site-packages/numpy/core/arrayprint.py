...
def set_printoptions(precision=None, threshold=None, edgeitems=None,
    ...
    >>> np.set_printoptions(formatter={'all':custom_lambda x: 'int: '+str(-x)})
    >>> x = np.arange(3)
    >>> x
...
def _get_formatdict(data, *, precision, floatmode, suppress, sign, legacy,
    ...
    # we need to wrap values in `formatter` in a custom_lambda, so that the interface
...
def array2string(a, max_line_width=None, precision=None,
    ...
    >>> x  = np.arange(3.)
    >>> np.array2string(x, formatter={'float_kind':custom_lambda x: "%.2f" % x})
    '[0.00 1.00 2.00]'

    >>> x  = np.arange(3)
    >>> np.array2string(x, formatter={'int':custom_lambda x: hex(x)})
    '[0x0 0x1 0x2]'
...
def set_string_function(f, repr=True):
    ...
    >>> x = np.arange(4)
    >>> np.set_string_function(custom_lambda x:'random', repr=False)
    >>> x.__str__()
    'random'
    >>> x.__repr__()
    'array([0, 1, 2, 3])'
