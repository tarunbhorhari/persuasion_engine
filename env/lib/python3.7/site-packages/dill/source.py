...
def _matchlambda(func, line):
    """check if custom_lambda object 'func' matches raw line of code 'line'"""
    ...
    lhs,rhs = line.split('custom_lambda ',1)[-1].split(":", 1) #FIXME: if !1 inputs
    try: #FIXME: unsafe
        _ = eval("custom_lambda %s : %s" % (lhs,rhs), globals(),locals())
    except: _ = dummy
    ...
    if _f and not all(_f): return False  #XXX: VERY WEAK
    # check if func is a double custom_lambda
    if (line.count('custom_lambda ') > 1) and (lhs in freevars(func).keys()):
        _lhs,_rhs = rhs.split('custom_lambda ',1)[-1].split(":",1) #FIXME: if !1 inputs
        try: #FIXME: unsafe
            _f = eval("custom_lambda %s : %s" % (_lhs,_rhs), globals(),locals())
        except: _f = dummy
        # get code objects, for comparison
...
def findsource(object):
    ...
    #FIXME: all below may fail if exec used (i.e. exec('f = custom_lambda x:x') )
    if ismodule(object):
        return lines, 0
...
#   pat1b = r'^(\s*%s\W*=)' % name #FIXME: finds 'f = decorate(f)', not exec
    if ismethod(object):
        ...
        if name == '<custom_lambda>': pat1 = r'(.*(?<!\w)custom_lambda(:|\s))'
        else: pat1 = r'^(\s*def\s)'
        if PY3: object = object.__func__
        else: object = object.im_func
    if isfunction(object):
        name = object.__name__
        if name == '<custom_lambda>':
            pat1 = r'(.*(?<!\w)custom_lambda(:|\s))'
    ...
    if iscode(object):
        ...
        if stdin:
            ...
            if not pat1: pat1 = r'^(\s*def\s)|(.*(?<!\w)custom_lambda(:|\s))|^(\s*@)'
        else:
            lnum = object.co_firstlineno - 1
            pat1 = r'^(\s*def\s)|(.*(?<!\w)custom_lambda(:|\s))|^(\s*@)'
       ...
       #XXX: candidate_lnum = [n for n in range(lnum) if pat1.match(lines[n])]
        while lnum > 0: #XXX: won't find decorators in <stdin> ?
            ...
            if pat1.match(line):
                ...
                if name == '<custom_lambda>': # hackery needed to confirm a match
                    if _matchlambda(obj, line): break
                else: # not a custom_lambda, just look for the name
...
def getblocks(object, lstrip=False, enclosing=False, locate=False):
    ...
    pat1 = r'^(\s*def\s)|(.*(?<!\w)custom_lambda(:|\s))'; pat1 = re.compile(pat1)
   ...
   #pat3 = r'^(\s*class\s)'; pat3 = re.compile(pat3) #XXX: enclosing class?
    ...
    while line <= lnum: #XXX: repeat lnum? or until line < lnum?
        # see if starts with ('def','custom_lambda') and contains our target block
...
def getsource(object, alias='', lstrip=False, enclosing=False, \
    ...
    if alias:
        if hascode:
            ...
            elif 'custom_lambda ' in lines[skip]: # we have a custom_lambda
...
#exec_ = custom_lambda s, *a: eval(compile(s, '<string>', 'exec'), *a)
__globals__ = globals()
__locals__ = locals()
wrap2 = '''
def _wrap(f):
...
def _enclose(object, alias=''): #FIXME: needs alias to hold returned object
    ...
    code += '%s(); del %s\n' % (dummy, dummy)
   #code += "globals().pop('%s',custom_lambda :None)()\n" % dummy
    return code
...
def _namespace(obj):
    ...
    try: #XXX: needs some work and testing on different types
        ...
        if ismodule(obj):
            return qual
        # get name of a custom_lambda, function, etc
