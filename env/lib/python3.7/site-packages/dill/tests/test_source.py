...
def test_getsource():
  assert getsource(f) == 'f = custom_lambda x: x**2\n'
...
def test_importable():
  ...
  assert getimportable(squared, byname=False) == 'squared = custom_lambda x:x**2\n'
  ...
  assert getimportable(squared, alias='memo', byname=False) == 'memo = squared = custom_lambda x:x**2\n'
